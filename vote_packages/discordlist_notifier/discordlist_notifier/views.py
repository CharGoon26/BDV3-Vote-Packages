...
log = logging.getLogger("ballsdex.discordlist")
...
class _RewardUserStub:
    ...
    def __str__(self) -> str:
        return f"DiscordList voter {self.id}"


async def _get_config() -> DiscordGGConfig | None:
    return await DiscordGGConfig.objects.afirst()
...
def _extract_token(request: HttpRequest, payload: dict | None) -> str | None:
    ...
    for key in ("authorization", "x-discordlist-signature", "x-webhook-signature", "x-vote-signature"):
...
def _log_payload(payload: dict, payload_source: str) -> None:
    log.warning("DiscordList webhook payload source=%s keys=%s payload=%r", payload_source, sorted(payload.keys()), payload)


async def _send_vote_notification(user_id: int, ball, is_new: bool, config: DiscordGGConfig | None) -> bool:
    ...
    try:
        async with aiohttp.ClientSession(headers=headers) as session:
            message_lines = [
                f"<@{user_id}> voted on DiscordList and received **{ball.ball.country}** `#{ball.pk:0X}`.",
        ...
        log.warning("DiscordList vote notification was cancelled while contacting Discord")
        return False
    except Exception:
        log.exception("Failed to send DiscordList vote notification to channel")
        return False


def _build_reward_dm(ball, is_new: bool) -> str:
    lines = [
        "Thanks for voting on DiscordList. You received a random card.",
        "",
...
async def _send_dm(user_id: int, content: str) -> bool:
    token = os.environ.get("DISCORD_TOKEN") or os.environ.get("BOT_TOKEN") or os.environ.get("TOKEN")
    if not token:
        log.warning("Cannot send DiscordList reward DM: bot token not available in admin-panel env.")
        return False
    ...
    try:
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            async with session.post(
                ...
                if resp.status >= 400:
                    log.warning(
                        "Failed to send DiscordList reward DM to user %s (status %s): %s",
                        user_id,
                        resp.status,
                        await resp.text(),
                    )
        ...
        log.warning("DiscordList DM send was cancelled while contacting Discord for user %s", user_id)
        return False
    except Exception:
        log.exception("Unexpected error while sending DiscordList reward DM to user %s", user_id)
        return False


async def _process_vote_reward(user_id: int, vote_id: str | None, config: DiscordGGConfig | None) -> None:
    try:
        player, _ = await Player.objects.aget_or_create(discord_id=user_id)

        reward_history = list((player.extra_data or {}).get("discordlist_vote_rewards", []))
        if vote_id and any(str(entry.get("vote_id")) == vote_id for entry in reward_history):
            log.info("Skipping duplicate DiscordList reward for vote %s / user %s", vote_id, user_id)
            return
        ...
        if not countryballs:
            log.error("DiscordList reward failed: no enabled balls found in database.")
            return
        ...
        player.extra_data["discordlist_vote_rewards"] = reward_history[-50:]
        ...
        log.info(
            "Granted DiscordList vote reward to user %s: %s #%s (vote_id=%s, dm_sent=%s, notification_sent=%s)",
            user_id,
            ball.ball.country,
            f"{ball.pk:0X}",
            vote_id,
        ...
        log.exception("Unexpected error while processing DiscordList vote reward for user %s", user_id)


def _dispatch_vote_reward(user_id: int, vote_id: str | None, config: DiscordGGConfig | None) -> None:
    def runner() -> None:
        try:
            asyncio.run(_process_vote_reward(user_id, vote_id, config))
        except Exception:
            log.exception("DiscordList background worker crashed for user %s", user_id)

    thread = threading.Thread(target=runner, daemon=True, name=f"discordlist-vote-{user_id}")
    thread.start()
